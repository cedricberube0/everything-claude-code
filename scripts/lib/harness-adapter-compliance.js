  ...
  [
    {
      ...
      harness: 'Claude Code',
      state: 'Native',
      supported_assets: ['Claude plugin assets', 'skills', 'commands', 'hooks', 'MCP config', 'local rules', 'statusline-oriented workflows'],
      unsupported_surfaces: ['Claude-native hooks do not imply parity in other harnesses'],
      install_or_onramp: ['`./install.sh --profile minimal --target claude`', 'Claude plugin install'],
      verification_commands: ['`npm run harness:audit -- --format json`', '`node scripts/session-inspect.js --list-adapters`'],
      risk_notes: ['Avoid loading every skill by default; keep hooks opt-in and inspectable.'],
      last_verified_at: '2026-05-12',
      owner: 'ECC maintainers',
      source_docs: ['.claude-plugin/plugin.json', 'docs/architecture/cross-harness.md', 'scripts/lib/install-targets/claude-home.js']
    },
    {
      id: 'codex',
      harness: 'Codex',
      state: 'Instruction-backed',
      supported_assets: ['`AGENTS.md`', 'Codex plugin metadata', 'skills', 'MCP reference config', 'command patterns'],
      unsupported_surfaces: ['Native hook enforcement and Claude slash-command semantics are not equivalent'],
      install_or_onramp: ['`./install.sh --profile minimal --target codex`', 'repo-local `AGENTS.md` review'],
    ...
    },
    {
      ...
      last_verified_at: '2026-05-12',
      owner: 'ECC maintainers',
      source_docs: ['.cursor/', 'scripts/lib/install-targets/cursor-project.js', 'tests/lib/install-targets.test.js']
    },
    {
      id: 'gemini',
      harness: 'Gemini',
      state: 'Instruction-backed',
      supported_assets: ['Gemini project-local instructions', 'shared skills', 'rules', 'compatibility docs'],
      ...
      last_verified_at: '2026-05-12',
      owner: 'ECC maintainers',
      source_docs: ['.gemini/', 'scripts/lib/install-targets/gemini-project.js', 'tests/lib/install-targets.test.js']
    },
    {
      id: 'zed',
      harness: 'Zed',
    ...
    },
    {
      ...
      harness: 'Pi',
      state: 'Native',
      supported_assets: ['skills (mapped via .pi/skills)', 'prompts (mapped via .pi/prompts)', 'subagents (mapped via .pi/agents & chains)', 'hooks (session_start, session_end)'],
      unsupported_surfaces: ['Deep Continuous Learning parity'],
      install_or_onramp: ['`pi install <repo_path>`', '`npm run build:pi`'],
      verification_commands: ['`npm run harness:adapters -- --check`'],
      risk_notes: ['Relies on build step to stay in sync with canonical sources.'],
      last_verified_at: '2026-06-24',
      owner: 'ECC maintainers',
      source_docs: ['docs/implementation/pi-integration-plan.md']
    },
    {
      id: 'dmux',
      harness: 'dmux',
      state: 'Adapter-backed',
      supported_assets: ['session snapshots', 'tmux/worktree orchestration status', 'handoff exports'],
      unsupported_surfaces: ['dmux is an orchestration runtime, not an install target for skills/rules'],
      install_or_onramp: ['`node scripts/session-inspect.js --list-adapters`', 'dmux session target inspection'],
      ...
      last_verified_at: '2026-05-12',
      owner: 'ECC maintainers',
      source_docs: ['scripts/lib/session-adapters/dmux-tmux.js', 'scripts/orchestration-status.js', 'tests/lib/session-adapters.test.js']
    },
    {
      id: 'orca',
      harness: 'Orca',
    ...
    },
    {
      ...
      harness: 'Ghast',
      state: 'Reference-only',
      supported_assets: ['terminal-native pane grouping', 'cwd grouping', 'search', 'notifications'],
      unsupported_surfaces: ['No ECC installer or direct adapter today'],
...
function renderMarkdownTable(records = ADAPTER_RECORDS) {
  ...
  for (const record of records) {
    lines.push(
      [record.harness, record.state, record.supported_assets, record.unsupported_surfaces, record.install_or_onramp, record.verification_commands, record.risk_notes]
...
function validateAdapterRecords(records = ADAPTER_RECORDS) {
  ...
  records.forEach((record, index) => {
    ...
    for (const field of ['supported_assets', 'unsupported_surfaces', 'install_or_onramp', 'verification_commands', 'risk_notes', 'source_docs']) {
