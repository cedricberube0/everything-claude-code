...
| Pi                 | Native             | skills (mapped via .pi/skills); prompts (mapped via .pi/prompts); subagents (mapped via .pi/agents & chains); hooks (session_start, session_end) | Deep Continuous Learning parity                                                | `pi install <repo_path>`; `npm run build:pi`                                      | `npm run harness:adapters -- --check`                                                       | Relies on build step to stay in sync with canonical sources.                             |
