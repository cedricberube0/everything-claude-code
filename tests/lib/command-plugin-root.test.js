...
test('auto-update command probes for the script it runs, not just scripts/lib', () => {
  // A hooks-runtime-only install places scripts/lib/utils.js in ~/.claude,
  // which satisfies the resolver's default probe even though
  // scripts/auto-update.js is absent there. The command's inline resolver
  // must probe for auto-update.js itself so partial installs don't shadow
  // the full plugin root.
